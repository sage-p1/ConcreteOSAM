...
class SmartAVLNode:
    __slots__ = ['hash_key', 'value', 'children', 'height', 'left_height', 
                 'right_height', 'balance', 'left_balance', 'right_balance']
    ...
    def __getstate__(self):
        lb = self.left_balance if self.left_balance is not None else -9999
        rb = self.right_balance if self.right_balance is not None else -9999
        c1 = self.children[0] if self.children and self.children[0] is not None else -1
        c2 = self.children[1] if self.children and self.children[1] is not None else -1
        
        metadata = struct.pack('<32s8i', 
                               self.hash_key if self.hash_key else b'\0'*32, 
                               self.height, self.balance, self.left_height, self.right_height, 
                               lb, rb, c1, c2)
        
        val = self.value
        
        if isinstance(val, int):
             val_bytes = b'\x04' + struct.pack('<q', val)

        elif hasattr(val, 'get_label') and val.get_label() == 'multiwrite':
             val_bytes = b'\x01' + struct.pack('<Q?', val.p.head, val.temp_copy)
             
        else:
             p_bytes = pickle.dumps(val)
             val_bytes = b'\x00' + p_bytes
        
        return metadata + val_bytes

    def __setstate__(self, state):
        meta_size = struct.calcsize('<32s8i')
        metadata = state[:meta_size]
        val_data = state[meta_size:]
        
        (self.hash_key, self.height, self.balance, 
         self.left_height, self.right_height, 
         lb, rb, c1, c2) = struct.unpack('<32s8i', metadata)
        
        self.left_balance = lb if lb != -9999 else None
        self.right_balance = rb if rb != -9999 else None
        self.children = [c1 if c1 != -1 else None, c2 if c2 != -1 else None]
        
        tag = val_data[0]
        
        if tag == 4:
            self.value = struct.unpack('<q', val_data[1:9])[0]
            
        elif tag == 1:
            from smart_pointer_multi_write import SmartPointerMultiWrite
            from smart_pointer import SmartPointer
            
            head, temp_copy = struct.unpack('<Q?', val_data[1:10])
            mw = SmartPointerMultiWrite(head)
            self.value = SmartPointer(mw, temp_copy=temp_copy)
            
        else:
            self.value = pickle.loads(val_data[1:])
