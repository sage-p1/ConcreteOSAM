...
class SmartPointer:
    ...
    def __getstate__(self):
        if hasattr(self.p, 'label') and self.p.label == 'multiwrite':
            return (1, self.p.head, self.temp_copy)
        else:
            return (0, self.p, self.temp_copy)

    def __setstate__(self, state):
        tag = state[0]
        if tag == 1:
            _, head, temp_copy = state
            from smart_pointer_multi_write import SmartPointerMultiWrite
